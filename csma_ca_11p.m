...
AverageArrivalTime = 50;    %平均到达时间 slottime
% AveragePacketLength = 100;    %平均帧长
PacketLength = 1536; % 固定帧长 slottime 1024*8bit帧长 = 204.8 slottime 帧长
CWnumber = 192;
Buffer_Threshold = 8*10^6/(PhyRate*SlotTime);%缓存门限
maxNumber = (maxCarNumber-minCarNumber)/numberPace+1;
sResult = zeros(1,maxNumber); %记录结果用
maxInsNode = zeros(1,maxNumber);

for loopM = 1:maxNumber %M工作站数目

M = minCarNumber + (loopM-1)*numberPace;
...
for t = 1:TotalTime
    ...
    NormDis = norm(activeArray,1) - norm(Collision,1);
...
% loopM = (M-minCarNumber)/numberPace+1;
sResult(loopM) = Throughput;
maxInsNode(loopM) = maxSimulNode;
end
% matlabpool close;
...
grid;
save([num2str(floor(roadLength/1000)),'k',num2str(totalSecond),'s',num2str(maxCarNumber),'maxnode'],'maxInsNode');
save([num2str(floor(roadLength/1000)),'k',num2str(totalSecond),'s',num2str(maxCarNumber),'sResult'],'sResult');